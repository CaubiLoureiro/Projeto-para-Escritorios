	...
	public void run() {
		try {
			...
			while(true){
				...
				if(menssagem.equalsIgnoreCase(Comandos.SAIR)){
					this.cliente.close();
				}else if(menssagem.startsWith(Comandos.MENSAGEM)){
					String nomeDestinario = menssagem.substring(Comandos.MENSAGEM.length(), menssagem.length());
...
//						escritor.println("digite uma mensagem para " + destinario.getNomeCliente());
				...
				}else if(menssagem.equals(Comandos.LISTA_USUARIOS)){
					atualizarUsuarios(this);
				}else{
					escritor.println(this.nomeCliente + ", você disse: " + menssagem);
				}
			}
		...
		while(true){
			...
			if(this.nomeCliente.equalsIgnoreCase("null") || this.nomeCliente.isEmpty()){
				...
				for(String cliente: clientes.keySet()){
					atualizarUsuarios(clientes.get(cliente));
				}
				break;
			}
		}
	...
	private void atualizarUsuarios(GerenciadorDeClientes cliente) {
